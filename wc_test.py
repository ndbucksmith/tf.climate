...
def examp_plot(bx, name):
    ...
    tabl_ = [
        [
            round(ins[bx, 0], 1),
            round(ins[bx, 1], 1),
            round(meta_ests[bx], 1),
            round(meta_yt[bx], 1),
            round(trus[bx], 1),
            round(meta_errs[bx], 3),
        ],
        np.ma.round(ins[bx, 2:8], 2),
        np.ma.round(ins[bx, 8:14], 2),
        np.ma.round(ins[bx, 14:20], 2),
    ]
...
for mcx in range(1):
    ...
    for tx in range(50):
        ...
        sq_errs, ests, yt, meta_ests, meta_sqerrs, meta_yt, dodis, do_dins = sess.run(
            fetch, feed
        )
        ...
        surfPower_grad = wc_radCon * dodis[:, :, wc_start].sum(
            axis=1
        )  # (dodis[:,:,gs_radx].sum(axis=1) + (75.0 * dodis[:,:,wc_start].sum(axis=1))  \
        ...
        def wiggle_power(wig):
            ...
            if True:  # wc_radx != None:
                feed[rmdl.xin][:, :, wc_start] = feed[rmdl.xin][:, :, wc_start] + (
                    wig * wc_radCon
                )
                feed[rmdl.xin][:, :, wc_start + 2] = feed[rmdl.xin][
                    :, :, wc_start + 2
                ] + (wig)

        def wiggle_prep(factr):
            feed[rmdl.xin][:, :, 7] = feed[rmdl.xin][:, :, 7] * factr
            feed[rmdl.xin][:, :, wc_start + 1] = (
                feed[rmdl.xin][:, :, wc_start + 1] * factr
            )
        ...
        gtu.arprint(
            [dTdP.mean(), dTdP.max(), dTdP.min(), dTdP.std(), (dTdP < 0.0).sum()]
        )
        gtu.arprint(
            [
                meta_dTdP.mean(),
        ...
        if False:  # graph to compare manaul sens calc and tf.grads
            f1sens, a1sens = scat(
                ...
                "k",
                None,
                "Manual Sens calc with  delta="
                + str(watts)
                + " watts vs. tf.gradients",
                None,
            )
            a1sens.xaxis.set_label_text("manual")
            a1sens.yaxis.set_label_text("tf.gradient")
            f1sens.text(
                0.2, 0.8, "units are degree C/wm2", transform=plt.gcf().transFigure
            )
            plt.show()


test_map = np.array(test_map)
tm_ch_list = wcb.nn_features + [
    "err1",
    "err3",
    "err3",
    "err4",
...
fig_inf_ct.suptitle(
    "tf.climate meta-model number 1 and 2 gradients, hot and cold count by normalized feature"
)
cowi = 0.4
...
axe21.bar(
    range(len(sen_histo[0])), sen_histo[0], align="center",
)
ticklbls = []
...
f2, a2 = mapper(
    exx, wye, zee, "coolwarm", "albedo corrected sensitivity map", sen_normalize
)
