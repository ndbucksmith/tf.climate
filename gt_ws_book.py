...
unit_x = 10; unit_y = 10  #30 second units
u15x = 2 * unit_x; u15y = 2* unit_y;  #15 second units
...
N_polar_ct = 0,0; S_polar_ct = 0,0;
N_temp_ct = 0,0; S_temp_ct = 0.0;
N_tropic_ct = 0,0; S_tropic_ct = 0,0;
...
rg00 = range(0,7200, u15x); rg01 = range(7200, 10800-u15x, u15x)
rg10 = range(0, 3600, u15x); rg11 = range(3600, 10800-u15x, u15x) 
    ...
    for d3lax in range(0, 10800-u15x, u15x):
      ...
      #Window(col_off, row_off, width, height)
        ...
        if hi_datact > 3 and te_datact > 3 and lc_datact > 3 and el_datact > 3:
          ...
          if to: zone = d3_zones[d3row][0]
          else: zone =  d3_zones[d3row][1]
          ...
          if np.random.random() < 0.9:
            ...
            trainct += 1;  bTrain = True;
            ins, b_g, r_s, t_12, wc_t, d3i = wcb.bld_eu_examp(d3ws, wcws, lcws, d3lax, d3lox, lon, lat, tilx, True)
          else:
            teix.append([d3lax, d3lox, lon, lat, elev, hi_datact, te_datact, lc_datact, \
                       unit_x, unit_y, lc_unit_x, lc_unit_y]);
            testct += 1; bTrain = False
            ins, b_g, r_s, t_12, wc_t, d3i = wcb.bld_eu_examp(d3ws, wcws, lcws, d3lax, d3lox, lon, lat, tilx, False)
          def fmlat(co):
            if co > 0.0: 
          ...
          if b_g:
            if bTrain:
              zoneTr_cts[zone] += 1
              fpath = targ + zone + '/train/ex_' + str(zoneTr_cts[zone]) + fmlon(lon) + fmlat(lat) + '.pkl'
            else:
              zoneTe_cts[zone] += 1
              fpath = targ + zone + '/test/ex_' + str(zoneTe_cts[zone]) + fmlon(lon) + fmlat(lat) + '.pkl'            
            exD = {'coords':[d3lax, d3lox, lon, lat, tilx], 'ins': ins, 'r_s':r_s, 't_12':t_12, 'wc_t':wc_t}
